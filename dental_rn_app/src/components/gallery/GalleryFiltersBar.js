...
import { AI_STATUSES, DIAGNOSIS_OPTIONS, GALLERY_DOCTORS, SCAN_TYPES, SORT_OPTIONS } from '../../theme/galleryTokens';
import { colors, radii, spacing, typography } from '../../theme/tokens';

const DATE_OPTIONS = ['All Dates', 'Today', 'This Week', 'This Month'];

export default function GalleryFiltersBar({ searchQuery, onSearchChange, filters, onChangeFilter, patientNames, sort, onChangeSort }) {
  const [openKey, setOpenKey] = useState(null);

  const filterDefs = [
    { key: 'patient', label: 'Patient', options: patientNames },
    { key: 'date', label: 'Date', options: DATE_OPTIONS },
    { key: 'aiStatus', label: 'AI Status', options: AI_STATUSES },
    { key: 'diagnosis', label: 'Diagnosis', options: DIAGNOSIS_OPTIONS },
    { key: 'doctor', label: 'Doctor', options: GALLERY_DOCTORS },
    { key: 'scanType', label: 'Scan Type', options: SCAN_TYPES },
  ];
    ...
    <View style={{ marginBottom: spacing.md }}>
      <View style={styles.searchRow}>
        ...
        <TextInput
          ...
          placeholder="Search by patient, patient ID, scan ID, treatment, or doctor..."
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chipRow}>
        <TouchableOpacity
          ...
          onPress={() => onChangeSort(sort === 'Newest First' ? 'Oldest First' : 'Newest First')}
        >
